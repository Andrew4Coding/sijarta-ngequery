...
export async function POST(req: Request) {
  const { userId, role, trPemesananJasaId }: ApplyPekerjaanInterface =
    await req.json();
  ...
  const pemesananJasa = await new TrPemesananJasa().findBy(
    "id",
    trPemesananJasaId
  );
  ...
  await new TrPemesananStatus().update("idtrpemesanan", pemesananJasa.id, {
  ...
  await new TrPemesananJasa().update("id", trPemesananJasaId, {
    ...
    waktupekerjaan: new Date(
      new Date().getTime() + Number(pemesananJasa.sesi) * 24 * 60 * 60 * 1000
    ),
    idpekerja: userId,
  });
